...
describe('Trade', () => {
  const token0 = new Token(ChainId.FANTOM, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.FANTOM, '0x0000000000000000000000000000000000000002', 18, 't1')
  const token2 = new Token(ChainId.FANTOM, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.FANTOM, '0x0000000000000000000000000000000000000004', 18, 't3')

  const pair_0_1 = new Pair(new TokenAmount(token0, JSBI.BigInt(1000)), new TokenAmount(token1, JSBI.BigInt(1000)), ChainId.FANTOM)
  const pair_0_2 = new Pair(new TokenAmount(token0, JSBI.BigInt(1000)), new TokenAmount(token2, JSBI.BigInt(1100)), ChainId.FANTOM)
  const pair_0_3 = new Pair(new TokenAmount(token0, JSBI.BigInt(1000)), new TokenAmount(token3, JSBI.BigInt(900)), ChainId.FANTOM)
  const pair_1_2 = new Pair(new TokenAmount(token1, JSBI.BigInt(1200)), new TokenAmount(token2, JSBI.BigInt(1000)), ChainId.FANTOM)
  const pair_1_3 = new Pair(new TokenAmount(token1, JSBI.BigInt(1200)), new TokenAmount(token3, JSBI.BigInt(1300)), ChainId.FANTOM)

  const pair_weth_0 = new Pair(
    new TokenAmount(WFTM[ChainId.FANTOM], JSBI.BigInt(1000)),
    new TokenAmount(token0, JSBI.BigInt(1000)), ChainId.FANTOM
  )

  const empty_pair_0_1 = new Pair(new TokenAmount(token0, JSBI.BigInt(0)), new TokenAmount(token1, JSBI.BigInt(0)), ChainId.FANTOM)

  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], FTM),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(FTM)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with ETHER as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], FTM, token0),
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, FTM),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(FTM)
  })
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(FTM)
      expect(result[0].route.path).toEqual([WFTM[ChainId.FANTOM], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(FTM)
      expect(result[1].route.path).toEqual([WFTM[ChainId.FANTOM], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WFTM[ChainId.FANTOM]])
      expect(result[0].outputAmount.currency).toEqual(FTM)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WFTM[ChainId.FANTOM]])
      expect(result[1].outputAmount.currency).toEqual(FTM)
    })
  })
